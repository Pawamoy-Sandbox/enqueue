...
// return the size
inline int getQSize(){return MAXQUEUE;}
...
//~ Queue clone(Queue q){
	//~ Queue cq = newQueue(getQSize());
	//~ setQHead(cq, getQHead(q));
	//~ setQTail(cq, getQTail(q));
	//~ setQSize(cq, getQSize());
	//~ copyValues(q, cq);
	//~ return cq;
//~ }
...
void deleteQueue(Queue q){
	if (getQQueue(q)!=NULL) free(getQQueue(q));
	return ;
} 
...
// return the head
// !empty(q)
inline SQueue getHead(Queue q){return getQQueue(q)[getQHead(q)%getQSize()];}
// return the tail
// !empty(q)
inline SQueue getTail(Queue q){return getQQueue(q)[(getQTail(q)-1)%getQSize()];}
// return the element k
// !empty(q)
inline SQueue getElem(Queue q, int k){return getQQueue(q)[(getQHead(q)+k)%getQSize()];}
// return the length
...
void setHead(Queue q, SQueue x){
	getQQueue(q)[getQHead(q)%getQSize()] = x;
}
// set the tail at x
// !empty(q)
void setTail(Queue q, SQueue x){
	getQQueue(q)[(getQTail(q)-1)%getQSize()] = x;
}
// set the element k at x
// !empty(q)
void setElem(Queue q, SQueue x, int k){
	getQQueue(q)[(getQHead(q)+k)%getQSize()] = x;
}
// add x in head
// !filled(q)
void addHead(Queue q, SQueue x){
	...
	if (getQHead(q)<0) {
		setQHead(q, getQSize()+getQHead(q));
		setQTail(q, getQTail(q)+getQSize());
	}
	getQQueue(q)[getQHead(q)%getQSize()] = x;
}
// add x in tail
...
void addTail(Queue q, SQueue x){
	getQQueue(q)[getQTail(q)%getQSize()] = x;
...
// queue is filled ?
inline Bool filled(Queue q){return (getLength(q)==getQSize());}
